...
\subsection{Betrachtung der Deskriptorlänge}\label{pca_experiments}
...
%warum man das zusammen mit thresh betrachten sollte
%erläuterung wie thresh gewählt wird. max abstand ist 2.0, da norimiert, aber der durchschnittliche Abstand steigt mit mehr varianz, daher muss der threshold auch angepasst werden, damit das initiale matching auch deskriptoren, die keinen validen partner haben aussortiert
% dann einfluss aufzeigen mit plot, von thresh und expl. varianz
% dann ergebnisse besprechen. weniger ist aufjedenfall nicht so gut, mehr bringt nur wenig. auf hist daten ist der unterschied noch größer, vielleicht lieber 80 dim benutzen. 
%dann idee, da die beiden parameter abhängig sind vielleicht thresh an erklärte varianz koppeln. funktioniert gutisch so im bereich 0.6 gibts die besten ergebnisse. Fußnote, dass das bei reinen feature matches natürlich direkter ins gewicht fällt und auch besser zu sehen ist mit verweise auf plots im anhang