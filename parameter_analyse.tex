...
Obwohl sich Klassifikations- und Retrievalaufgaben im Kern ähneln, können viele Metriken mit denen Klassifikationssysteme üblicherweise bewertet werden, wie beispielsweise Genauigkeit, nicht verwendet werden, um die Performanz von Retrievalsystemen zu evaluieren. Generell genügen die berechneten Ähnlichkeiten zwischen einzelnen Bildpaaren bestehend aus Anfragebild und Bild aus dem Suchindex nicht, um eine Aussage über die Performanz eines Retrievalsystems zu treffen. Stattdessen sind diese im Kontext aller möglichen Bildkombinationen aus Anfragebild und Suchindex zu betrachten, wobei das resultierende Ranking der Paare Grundlage für die Bewertung eines Systems ist. Eine Möglichkeit der Bewertung eines Rankings ist die Erstellung einer sogenannten ROC-Kurve (Reciever-Operating-Characteristic)(vgl. Abb. \ref{metric_curve}a). Hierfür wird eine wachsende Anzahl der zurückgegeben Bilder, sortiert nach der vom Ranking festgelegten Reihenfolge, betrachtet und jeweils das Verhältnis zwischen Richtig-Positiv-Rate bzw. Recall und Falsch-Positiv-Rate dargestellt. Der Recall gibt an, welcher Anteil an Bildern mit gewünschten Bildinhalt bereits im betrachteten Abschnitt der Rückgabe enthalten war.
\begin{equation}
...
\end{equation}
Analog betrachtet gibt die Falsch-Positiv-Rate den Anteil der bereits zurückgegebenen Bilder ohne gewünschten Bildinhalt an.
\begin{equation}
...
\end{figure}
\\
ROC-Kurven beginnen stets im Ursprung, wo weder Bilder mit noch ohne korrektem Bildinhalt zurückgegeben wurden und enden im Punkt $(1,1)$ wo der gesamte Datensatz zurückgegeben wurde. Die Gerade zwischen diesen Punkten bildet den erwarteten Verlauf, wenn das System Bilder in einer zufälligen Reihenfolge zurückgibt. Ein Retrievalsystem hat also nur dann einen positiven Effekt wenn die ROC-Kurven ihrer Anfrageantworten oberhalb dieser Linie verlaufen. Obwohl ROC-Kurven gut in der Lage sind den Unterschied eines Suchsystems gegenüber einer zufälligen Suche aufzuzeigen, vermitteln sie aus der Perspektive eines tatsächlichen Anwenders oft ein zu positives Bild der Ergebnisse. Typischerweise ist für eine Suchanfrage die Anzahl der relevanten Bilder um ein vielfaches geringer als die Anzahl der irrelevanten Bilder. Angenommen für eine Suchanfrage sind $10\%$ eines durchsuchten Datensatzes tatsächlich relevant. Die zur Anfrage erstellte ROC-Kurve zeigt bei einem Recall von $90\%$ eine Falsch-Positiv-Rate von $30\%$. Was zunächst wie ein gutes Ergebnis wirkt, bedeutet für den Nutzer, dass $75\%$ der Ergebnisse, die er erhalten hat, bis $90\%$ der für ihn relevanten Ergebnisse gefunden wurden für ihn keine Relevanz haben.
Um einen realistischeren Eindruck der Nutzbarkeit darzustellen, werden für Retrievalsysteme häufig PR-Kurven (Precision-Recall) erstellt (vgl. Abb. \ref{metric_curve}b). Hierbei wird der Recall dem Anteil der relevanten Bildern in der bisherigen Rückgabe gegenübergestellt.
\begin{equation}
\text{Precision} = \frac{|\text{Bereits züruckgebene Bilder mit gewünschtem Inhalt}|}{|\text{Bereits züruckgebene Bilder}|}
\end{equation}
Die vorgestellten Kurvenmetriken eigenen sich gut um einzelne Anfragen an Retrievalsysteme genau zu analysieren und zu visualisieren. Um Retrievalsysteme in Gänze, auf Basis mehrerer Anfrage zu bewerten und mit anderen Systemen zu vergleichen, bietet es sich an ein kompaktere Metriken zu verwenden. Hierfür wird die Fläche unterhalb der Metrikkurven betrachtet. AUC (Area Under Curve) und AP (Average Percision) beschrieben jeweils die Flächen unterhalb von ROC bzw. PR-Kurven und können so eine Anfrageantwort mit einer einzelnen Zahl bewerten. Auf Grund der besseren Interpretierbarkeit für den Endnutzer werden in dieser Arbeit PR-Kurven bzw. AP-Werte zur Auswertung genutzt. Für den Vergleich zwischen unterschiedlichen Retrievalsystemen oder Konfigurationen von DELF werden die AP-Werte von mehreren Anfragen an ein System zu einem Mittelwert (mAP) zusammengefasst. 
