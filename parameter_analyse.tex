...
Obwohl sich Klassifikations- und Retrievalaufgaben im Kern ähneln, können Metriken wie Genauigkeit, mit der Klassifikationssysteme üblicherweise bewertet werden, nicht verwendet werden, um die Performanz von Retrievalsystemen zu evaluieren. Generell genügt die berechnete Ähnlichkeit zwischen einzelnen Bildpaaren bestehend aus Anfragebild und Bild aus dem Suchindex nicht, um eine Aussage über die Performanz eines Retrievalsystems zu treffen. Stattdessen ist diese im Kontext aller möglichen Bildkombinationen aus Anfragebild und Suchindex zu betrachten, wobei das resultierende Ranking der Paare Grundlage für die Bewertung des Systems ist. Eine Möglichkeit der Bewertung eines Rankings ist die Erstellung einer sogenannten ROC-Kurve(Reciever-Operating-Characteristic)(vgl. Abb. \ref{metric_curve}). Hierfür wird eine wachsende Anzahl der zurückgegeben Bilder, sortiert nach der vom Ranking festgelegten Reihenfolge, betrachtet und jeweils das Verhältnis zwischen Richtig-Positiv-Rate bzw. Recall und Falsch-Positiv-Rate dargestellt. Der Recall gibt an, welcher Anteil an Bildern mit gewünschten Bildinhalt bereits im betrachteten Abschnitt der Rückgabe enthalten waren.
\begin{equation}
...
\end{equation}
Analog betrachtet die Falsch-Positiv-Rate den Anteil der bereits zurückgegebenen Bilder ohne gewünschten Bildinhalt.
\begin{equation}
...
\end{equation}
ROC-Kurven beginnen stets im Ursprung, wenn weder Bilder mit noch ohne korrektem Bildinhalt zurückgegeben wurden und enden im Punkt $(1,1)$ wenn der gesamte Datensatz zurückgegeben wurde. Die Strecke zwischen diesen Punkten bildet den erwarteten Verlauf, wenn das System Bilder in einer zufälligen Reihenfolge zurückgibt. Ein Retrievalsystem hat also nur dann einen positiven Effekt wenn die ROC-Kurven ihrer Anfrageantworten überhalb dieser Linie verlaufen.
%
