...
\end{table}
In Bezug auf die Retrievalergebnisse auf den historischen Daten, erzielen beide Versionen des DELF-Verfahrens auf den $42$ untersuchten Anfrage insgesamt eine um Rund $0.06$ höhere mAP als das ConvNet-Verfahren, wobei sich je nach Objektkategorie teilweise starke Unterschiede zeigen. Hierbei gibt es einige Kategorien, wie die Semperoper oder Sophienkirche, die für alle getesteten Verfahren eine besonders große Herausforderung darstellen. Neben Aspekten wie Auflösung uns Aufnahmequalität gibt es speziell historische Einflüsse, die die Retrievalaufgabe für die Kategorien besonders schwierig gestaltet. Betrachtet man die Anfragebilder, welche mit den getesteten Verfahren am schlechtesten beantwortet wurden (vgl. Abb. \ref{worst_queries}), so sieht man, dass sich die hier gezeigten Aufnahmen der Semperoper und des Stallhofs architektonisch deutlich von ihrem heutigen Selbst unterscheiden. Innerhalb des Datensatzes sind unterschiedliche bauliche Stadien, dieser Sehenswürdigkeiten enthalten, was ein genaues Matching erschwert. Auch zahlreiche Abbildungen von zerstörten Objekten, wie beispielsweise der Sophienkirchen sind ihren intakten Versionen nur schwer zuordenbar.  
\\
\begin{figure}[h]
\centering
\includegraphics[scale=0.40]{worst_queries}
\caption{Anfragebilder, die unter Verwendung der unterschiedlichen Verfahren die niedrigste AP erreichen.}
\label{worst_queries}
\end{figure}
In Abbildung \ref{destruction_pr} ist der Verlauf einer Suchanfrage der Sophienkirche der unterschiedlichen Verfahren an Hand von PR-Kurven dargestellt. Bei der Sophienkirche handelt es sich um die Kategorie, welche im Datensatz prozentual am häufigsten (teilweise) zerstört dargestellt ist (vgl. Tab. \ref{challenging_imgs}). Es stellt sich heraus, dass die Abbildungen der zerstörten Sophienkirchen zu einem überwiegenden Teil erst sehr spät innerhalb der Anfrageantworten zurückgegeben werden, wenn die Precision bereits sehr stark gesunken ist, bzw. schon viele Bilder ohne korrekten Bildinhalt zurückgegeben wurden. Daraus lässt sich schließen, dass Aufnahmen von zerstörten Gebäuden tatsächlich sehr viel schwerer ihren intakten Originalen zugeordnet werden können, als andere unbeschädigten Aufnahmen ihrer selbst.
\begin{figure}[h]
\centering
...
%beispiel nacht und luftbilder
%weitere probleme, die im datensatz auftreten sind nachtaufnahmen und luftbilder.
\begin{figure}[h]
\centering
...
\label{destruction_pr}
\end{figure}
%dabei zeigt sich, dass delf mehr probleme mit dunkelheit hat danach verlauf bika mit is_night
\begin{figure}[h]
\centering
...
\label{destruction_pr}
\end{figure}
%% vielleicht kann man die pr_curven noch länglicher plotten, damit die seite besser gefüllt wird + man besser sieht
%liegt vermutlich mit daran, dass die selektion schon schwierigkeiten hat, wegen weniger details im bild -> darkness plots

% da delf besonders leidet nicht verwunderlich, dass besonders bei zwinger conv besser ist -> verweis auf effekttabelle (zwinger hat meiste nachtbilder)
\begin{table}[h]
\resizebox{\textwidth}{!}{%
...
%luftbilder sind für alle ein problem, da bildinhalt klein, ungewöhnliche perspektive und viel drum rum
% allerdings leidet conv stärker darunter. zeige wieder lowest tp und danach pr kurve von semperoper
% conv muss gesamten bildinhalt matchen, delf kann auswahl treffen -> unwichtige bereiche fallen nicht ins gewicht. Für sehr markante 
% gebäude hilft ransac auch noch mal ein sehr.
\begin{figure}[h]
\centering
